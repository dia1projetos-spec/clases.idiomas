...
onAuthStateChanged(auth, async user => {
  ...
  onSnapshot(query(collection(db,"aulas"), where("alunos","array-contains", user.uid)), s=>{
    ...
    div.innerHTML = '';
    s.forEach(d=>{ const a=d.data(); div.innerHTML+=`<div style="background:#fff;padding:12px;margin:8px 0;border-radius:8px"><b>${a.titulo}</b><br><iframe width="100%" height="200" src="${a.youtube}" frameborder="0" allowfullscreen></iframe><p>${a.desc||''}</p></div>`; });
  });
  onSnapshot(query(collection(db,"provas"), where("alunos","array-contains", user.uid)), s=>{
    const div = document.getElementById('minhasProvas');
    div.innerHTML='';
    s.forEach(d=>{ const p=d.data(); div.innerHTML+=`<div style="background:#fff;padding:12px;margin:8px 0;border-radius:8px">${p.titulo}</div>`; });
  });
  onSnapshot(collection(db,"biblioteca"), s=>{
    const div = document.getElementById('bibAluno');
    div.innerHTML='';
    s.forEach(d=>{ const b=d.data(); div.innerHTML+=`<div class="item"><img src="${b.capa}"><p>${b.titulo}</p></div>`; });
  });
});
