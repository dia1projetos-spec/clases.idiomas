...
onAuthStateChanged(auth, async user=>{
  ...
  uploadFoto.onchange = async e=>{
    try{
      ...
      if(!file) return;
      if(cloudinaryConfig.cloudName.includes("SEU_")){
        alert('Configure seu Cloudinary em js/firebase-config.js primeiro!');
        return;
      }
      ...
      if(!data.secure_url) throw new Error(data.error?.message || 'Erro Cloudinary');
